  ...
  <div>
    <div class="container" :style='{"minHeight":"100vh","alignItems":"center","background":"url(http://codegen.caihongy.cn/20230929/286f0f899e854f018a0be9e85e43d46d.jpg)","display":"flex","width":"100%","backgroundSize":"100% 100%","backgroundPosition":"center center","backgroundRepeat":"no-repeat","justifyContent":"center"}'>
      <el-form :style='{"border":"1px solid #f6f6f6","minHeight":"600px","padding":"30px 45% 20px 5%","margin":"50px auto 50px","borderRadius":"8px","textAlign":"center","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20230929/b96e0afa9486481d971edb8e706fdbe8.jpg) no-repeat right center / 50% 100%,#fff","display":"flex","width":"80%","fontSize":"14px","height":"auto"}'>
        <div v-if="true" :style='{"padding":"0px","margin":"0px auto 10px","borderColor":"#ddd","color":"#1e3c4f","textAlign":"center","display":"inline-block","background":"none","borderWidth":"0px","width":"100%","lineHeight":"40px","fontSize":"24px","borderStyle":"solid","fontWeight":"600"}' class="title-container">商品价格分析系统登录</div>
        <div v-if="登录Type==1" class="list-item" :style='{"width":"100%","margin":"0 auto 20px","position":"relative","alignItems":"center","flexWrap":"wrap","display":"flex"}'>
        ...
        <div :style='{"width":"100%","margin":"20px auto 0","fontSize":"inherit","textAlign":"center"}' v-if="roles.length>1" prop="登录InRole" class="list-type">
          <el-radio v-if="登录Type==1||(登录Type==2&&item.roleName!='管理员')" v-for="item in roles" v-bind:key="item.roleName" v-model="rulesForm.role" :label="item.roleName">{{item.roleName}}</el-radio>
        </div>
        ...
        <div :style='{"margin":"0px auto 0","alignItems":"center","flexWrap":"wrap","display":"flex","width":"100%","fontSize":"16px","position":"relative","justifyContent":"center"}'>
          <el-button v-if="登录Type==1" :style='{"border":"0","cursor":"pointer","padding":"0px","margin":"0 auto 20px","color":"#fff","textAlign":"center","outline":"none","borderRadius":"0px","background":"#1e3c4f","width":"100%","fontSize":"16px","fontWeight":"600","height":"50px","order":"2"}' type="primary" @click="登录()" class="登录InBt">登录</el-button>
          <el-button :style='{"border":"0px solid rgba(64, 158, 255, 1)","cursor":"pointer","padding":"0 8px","margin":"0 0px 10px","borderColor":"#000a42","color":"#000a42","outline":"none","borderRadius":"0px","background":"none","borderWidth":"1px","width":"100%","fontSize":"inherit","borderStyle":"solid","fontWeight":"600","height":"50px","order":"3"}' type="primary" @click="register('yonghu')" class="register">注册用户</el-button>
        </div>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    //注册
    register(tableName){
		this.$storage.set("登录Table", tableName);
	...
	登录Post() {
		this.$http({
			url: `${this.tableName}/登录?username=${this.rulesForm.username}&password=${this.rulesForm.password}`,
			method: "post"
		}).then(({ data }) => {
			if (data && data.code === 0) {
